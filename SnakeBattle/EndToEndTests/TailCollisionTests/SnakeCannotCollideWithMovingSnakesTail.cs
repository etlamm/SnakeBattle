...
{
    class SnakeCannotCollideWithMovingSnakesTail : IEndToEndTest
    {
        private Game game;
        private MainView mainView;

        public string Category => Categories.TailCollision;
        public string Description => "Snake cannot collide with moving snake's tail";
        ...
        public async Task Run()
        {
            ...
            game.AddPlayers(
                new AddPlayerRequest(new TurningPlayer(Direction.East, 10), Location.At(center.X - 5, center.Y - 5), Direction.East),
                new AddPlayerRequest(new TurningPlayer(Direction.East, 10), Location.At(center.X + 5, center.Y + 5), Direction.West)
                );
